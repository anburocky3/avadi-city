...
export const WardProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const addComplaint = async (data: {
    ...
    formData.append("userWard", String(data.userWard));
    formData.append("incidentWard", String(data.incidentWard || data.userWard));
    formData.append("shareOnFeed", String(data.shareOnFeed));
    formData.append("lat", String(data.lat));
    formData.append("lng", String(data.lng));
    formData.append("title", data.title);
    formData.append("description", data.description);

    // Append up to 3 File objects
    ...
    const res = await fetch("/api/complaints", {
      method: "POST",
      credentials: "include", // Transmits avadi_session cookie
      body: formData, // Content-Type is auto-generated by browser
    });

    const result = await res.json();

    if (!res.ok) {
      throw new Error(result.message || "Failed to submit grievance report");
    }
